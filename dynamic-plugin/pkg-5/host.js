// 归档:pkg-4 宿主半(人民币默认价)。
// 相对 pkg-3:价格表全部换算为 CNY;账本字段 costUsd 更名为 cost。
// 客户端半(同包 client.js)仍读取旧字段 costUsd,导致渲染崩溃,由 pkg-5 修复。
// 本文件为演进史归档;可用的持久版见仓库根目录 token-billing.js。
return {
  apply(ctx) {
    const MAX_LEDGER = 1000
    const MAX_SESSIONS = 200

    // 单价表: CNY / 1M tokens。'*' 为兜底行。默认值参考 DeepSeek 官方人民币定价,可随时通过工具/RPC 修改。
    const DEFAULT_FALLBACK = { pattern: '*', input: 2, output: 8, cacheRead: 0.5, cacheWrite: 2 }
    const prices = [
      { pattern: 'deepseek-reasoner', input: 4, output: 16, cacheRead: 1, cacheWrite: 4 },
      { pattern: 'deepseek-chat', input: 2, output: 8, cacheRead: 0.5, cacheWrite: 2 },
    ...
    const totals = { calls: 0, cost: 0, inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, reasoningTokens: 0 }
    ...
    function record(entry) {
      ...
      if (typeof sid === 'string' && sid.length > 0) {
        ...
        if (agg === undefined) {
          agg = { sessionId: sid, calls: 0, cost: 0, inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheWriteTokens: 0, reasoningTokens: 0 }
      ...
      if (perSession.size > MAX_SESSIONS) {
        let cheapest = null
        for (const agg of perSession.values()) if (cheapest === null || agg.cost < cheapest.cost) cheapest = agg
    ...
    function summary() {
      ...
      for (const agg of perSession.values()) sessions.push({
        sessionId: agg.sessionId, calls: agg.calls, cost: roundCost(agg.cost),
      ...
      for (let i = ledger.length - 1; i >= 0 && recent.length < 12; i--) {
        ...
        recent.push({
          ...
          usage: e.usage, cost: roundCost(e.cost), durationMs: e.durationMs,
    ...
    function setPrice(args) {
      ...
      return { ok: true, message: 'price row updated: "' + pattern + '" (CNY per 1M tokens)' }
    }
    ...
    const tool = harness.defineTool({
      name: 'billing_report',
      description: 'Token billing report and configuration (session-scoped panel companion). Reports every model call observed with token usage and CNY cost (prices per 1M tokens); can also set/remove model price rows or reset the meter.',
      parameters: {
        type: 'object',
        properties: {
          ...
          input: { type: 'number', description: 'CNY per 1M tokens: uncached input.' },
          output: { type: 'number', description: 'CNY per 1M tokens: output.' },
          cacheRead: { type: 'number', description: 'CNY per 1M tokens: cached input read.' },
          cacheWrite: { type: 'number', description: 'CNY per 1M tokens: cached input write.' },
          reasoning: { type: 'number', description: 'CNY per 1M tokens: reasoning tokens (defaults to output price).' },
        },
        required: ['action'],
      },
      output: {
      ...
      async execute(args) {
        ...
        if (action === 'report') {
          ...
          const lines = [
            ...
            '- currency: CNY',
            '- calls: ' + t.calls + ', total cost: ¥' + t.cost.toFixed(4),
          ...
          if (s.sessions.length > 0) {
            ...
            for (const row of s.sessions.slice(0, 15)) lines.push('  - ' + String(row.sessionId).slice(0, 10) + ': ' + row.calls + ' calls, ¥' + row.cost.toFixed(4))
          } else {
        ...
        if (action === 'list-prices') {
          ...
          return { ok: true, message: 'price rows (CNY per 1M tokens):\n' + rows.join('\n') }
        }
    ...
    console.log('[tokbl] token billing meter active (CNY)')
  },
}
